...
pub fn resolveVcpkgPaths(
    ...
    if (target_os == .windows) {
        ...
        const vcpkg_triplet = vcpkg_triplet_opt orelse "x64-windows";
        ...
        if (vcpkg_root_opt) |vcpkg_root| {
            ...
            if (std.fs.cwd().access(classic_lib, .{})) |_| {
                ...
                if (std.fs.cwd().access(manifest_lib, .{})) |_| {
                    ...
                    @panic("Windows vcpkg path enabled, but could not find vcpkg libraries. Expected either <VCPKG_ROOT>/installed/<triplet>/lib or ./vcpkg_installed/<triplet>/lib");
                }
            }
        } else {
            if (std.fs.cwd().access(manifest_lib, .{})) |_| {
                ...
                @panic("Windows vcpkg path enabled, but vcpkg deps were not found. On Windows, run `vcpkg install --triplet x64-windows` in the repo root (manifest mode), or set VCPKG_ROOT / pass --vcpkg-root for classic mode.");
            }
        }
    } else {
        return .{
