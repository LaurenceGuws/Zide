...
- Kitty graphics has started its first real ownership split: top-level parse/control/reply policy now flows through `KittyProtocolOps`, payload transport/build through `KittyTransport`, placement mutation/query/dirty through `KittyPlacementOps`, and byte-store replacement/eviction/clear through `KittyStorageOps`. Shared kitty state/types, payload transport/build, placement-graph mutation/scroll/placement helpers, and byte-store lifecycle now live in dedicated `src/terminal/kitty/common.zig`, `src/terminal/kitty/transport.zig`, `src/terminal/kitty/placement_ops.zig`, and `src/terminal/kitty/storage_ops.zig` modules; the remaining smell in this area is the still-large protocol/delete/reply orchestration left in `graphics.zig`.
