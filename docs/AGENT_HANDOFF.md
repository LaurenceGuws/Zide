...
- Engineer-complete macro batch (Architect verdict pending): `AHW-B9` — slot-aware app-shell contract alignment (no tab behavior).
...
- If a macro batch is `in_progress`, execute only that batch. `AHW-B9` engineer
  work is complete; next engineer session starts from Architect refocus.
- Architect: review the `AHW-B9` super-gate packet in `docs/todo/android/implementation.md` and refocus the queue.
