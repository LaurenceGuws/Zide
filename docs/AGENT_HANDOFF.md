...
- `src/editor/editor.zig` large-file cleanup is active: search worker/highlighter logic now lives under `src/editor/search_highlight.zig`, selection/multi-caret/undo-selection state under `src/editor/selection_state.zig`, and cursor navigation/selection-extension behavior under `src/editor/navigation.zig`, with `editor.zig` shrinking toward a real facade over edit operations.
