...
- Investigation residue cleanup is active: probe-grade scroll/control trace logging was removed from core hot paths, protocol trace noise now defaults to debug level, high-frequency input/scrollback send-path logs were demoted out of default-info output, and raw CSI-byte tracing now defaults to debug.
