...
- Kitty graphics has started its first real ownership split: query (`a=q`), upload/store (`a=t`/`a=T`), placement requests (`a=p`), and the major delete selector families are now factored out of the top-level kitty parser flow, image teardown for evict/delete/replace now shares one lifecycle path, payload transport/build now shares one internal `KittyTransport` boundary, and placement mutation/query/dirty work now flows through `KittyPlacementOps`. The remaining broad smell in this area is that image storage state and placement graph ownership still coexist in one module.
