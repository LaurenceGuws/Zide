...
- Current top-of-queue focus: continue the post-`src/app` giant file splits, now moving from the reduced `src/editor/editor.zig` into `src/editor/syntax.zig`, then widget draw, renderer/font, and config parse, while collapsing low-value micro-files created by earlier extraction work.
...
- `src/editor/syntax.zig` large-file cleanup is now active too: syntax query loading, capture-kind mapping, and query-cache ownership now live under `src/editor/syntax_queries.zig`, leaving `syntax.zig` more focused on runtime highlight/injection execution.
