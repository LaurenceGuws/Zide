...
- protocol/parser seam is still only partially typed; the VT parser callback contract in `parser.zig`, top-level parser dispatch in `parser_hooks`, DCS/APC/OSC/CSI parser dispatch, OSC title/hyperlink/cwd/clipboard/semantic/palette/kitty-clipboard boundaries, and kitty paste-event emission/internal helper paths now run through explicit session facades/context structs (including OSC 5522 write/reply flow through an explicit `WriterFacade`). CSI reply emission (`DA`/`DSR`/`DECRQM`/window ops/color-scheme) now runs through `CsiWriter`, CSI query/reply branches through `QueryContext`, CSI `SM`/`RM` mode toggles through `ModeMutationContext`, and `dcs_apc` no longer bounces kitty APC payloads back through a public `TerminalSession.parseKittyGraphics(...)` wrapper. CSI `DECRQM` mode capture uses explicit snapshot structs, CSI `DECSTR` now runs through an explicit reset context, and CSI `applySgr` now runs through an explicit SGR context; remaining implicit seams are narrower but still exist in parts of CSI mutation policy, selected protocol entry wrappers, and some widget/backend boundaries.
- Kitty graphics has started its first real ownership split: top-level parse/control/reply policy now flows through `KittyProtocolOps`, payload transport/build through `KittyTransport`, placement mutation/query/dirty through `KittyPlacementOps`, and byte-store replacement/eviction/clear through `KittyStorageOps`. The remaining smell in this area is mostly file-level concentration: one large module still owns several internal subsystems even though the responsibilities are now factored.
