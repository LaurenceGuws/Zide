...
- Kitty graphics has started its first real ownership split: query (`a=q`), upload/store (`a=t`/`a=T`), placement requests (`a=p`), and the major delete selector families are now factored out of the top-level kitty parser flow, and image teardown for evict/delete/replace now shares one lifecycle path. Payload decode/load, partial accumulation, image build, storage, and placement graph updates still live in one broad module.
