...
- Current top-of-queue focus: runtime scheduling ownership cleanup, then input publication redesign, then widget input/draw policy reduction.
...
- Runtime scheduling ownership cleanup is now active: workspace owns active-frame observation (`activeFrameState()`) and per-frame poll publication (`PollFrameResult`), runtime owns poll profile selection (`PollPolicy`), and terminal idle/backoff bookkeeping now lives inside `TerminalFramePacingState` instead of generic `AppState` fields. Pacing/poll helpers no longer reconstruct scheduling state from multiple raw session-generation getters or app-side pre/post generation comparisons, and workspace polling no longer hardcodes the input-vs-idle budget table internally.
