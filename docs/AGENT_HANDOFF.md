...
- protocol/parser seam is still only partially typed; top-level parser dispatch in `parser_hooks`, DCS/APC/OSC/CSI parser dispatch, OSC title/hyperlink/cwd/clipboard/semantic/palette/kitty-clipboard boundaries, and kitty paste-event emission/internal helper paths now run through explicit session facades/context structs (including OSC 5522 write/reply flow through an explicit `WriterFacade`), CSI reply emission (`DA`/`DSR`/`DECRQM`/window ops/color-scheme) now runs through `CsiWriter`, and `dcs_apc` no longer bounces kitty APC payloads back through a public `TerminalSession.parseKittyGraphics(...)` wrapper. CSI `DECRQM` mode capture uses explicit snapshot structs, CSI `DECSTR` now runs through an explicit reset context, and CSI `applySgr` now runs through an explicit SGR context; remaining implicit `anytype` seams still exist in CSI internals, selected protocol entry wrappers, and especially kitty graphics.
