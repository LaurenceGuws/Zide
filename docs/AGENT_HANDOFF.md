...
- Workspace/session boundary tightening is now active: app clipboard shortcuts no longer resolve sessions through workspace, the dead app-side active-session resolver was deleted, and workspace tab creation now has an explicit `createTabWithSession(...)` contract instead of relying on `createTab(...)` plus a follow-up `activeSession()` lookup.
