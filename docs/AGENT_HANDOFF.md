...
- `src/editor/editor.zig` large-file cleanup has started: search worker, regex matching, search query replacement, and highlighter lifecycle logic now live under `src/editor/search_highlight.zig`, with `editor.zig` delegating through a facade instead of carrying that subsystem inline.
