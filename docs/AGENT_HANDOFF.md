...
- `TerminalSession` surface reduction is largely landed: borrowed title/cwd/scrollback/selection seams were cut back, terminal text export now lives in backend code instead of the widget, FFI/workspace/open-path callers use backend-owned metadata/export contracts instead of separate raw getter calls, split title/cwd helpers, or a dedicated child-exit getter, metadata/hyperlink/clipboard/close-confirm query logic now lives under `src/terminal/core/session_queries.zig`, and generation/backlog/presentation-feedback bookkeeping now lives under `src/terminal/core/session_publication.zig` instead of sitting inline in `terminal_session.zig`.
