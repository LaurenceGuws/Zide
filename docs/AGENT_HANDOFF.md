...
- Kitty graphics has started its first real ownership split: query (`a=q`), upload/store (`a=t`/`a=T`), placement requests (`a=p`), and the major delete selector families are now factored out of the top-level kitty parser flow, image teardown for evict/delete/replace now shares one lifecycle path, and payload transport/build now shares one internal `KittyTransport` boundary. Storage and placement graph updates still live in the same broad module.
