...
- Active macro batch (`in_progress`, super-gate `CZH-GATE-113`): `CZH-B59` in
...
- Active ticket source: `docs/todo/core/CZH_S54_TICKETS.md`
...
Current batch status (`CZH-B59`, in progress toward `CZH-GATE-113`):

- target: unify canonical entry/eligibility surfaces so each flow has one entry route and minimal boundary glue
- prune remaining duplicate helper exposure after result-only boundary contraction
...
4. `docs/todo/core/CZH_S54_TICKETS.md`
