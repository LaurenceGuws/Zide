...
- `src/editor/editor.zig` large-file cleanup is active: search worker/highlighter logic now lives under `src/editor/search_highlight.zig`, and selection/multi-caret/undo-selection state now lives under `src/editor/selection_state.zig`, with `editor.zig` shrinking toward a real facade over edit operations.
