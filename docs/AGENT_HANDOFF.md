...
- protocol/parser seam is still only partially typed; DCS/APC and OSC dispatch now run through explicit session facades, but CSI and most OSC sub-handler internals still rely on broader implicit `anytype self` contracts.
