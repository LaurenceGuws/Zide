...
- The terminal FFI contract has now also survived an external peer-review host check in Flutty: the same widget/runtime layer worked across bridge-owned PTY and Flutter-owned PTY transport, with the remaining known asymmetry limited to transport-side focus/color-scheme reporting on the Flutter-owned path.
