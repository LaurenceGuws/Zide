...
- Kitty graphics has started its first real ownership split: both the query path (`a=q`) and upload/store path (`a=t`/`a=T`) are now factored out of the top-level kitty parser flow, but payload decode/load, partial accumulation, image build, storage, placement graph updates, and delete policy still live in one broad module.
