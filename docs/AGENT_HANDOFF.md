...
- Kitty graphics has started its first real ownership split: query (`a=q`), upload/store (`a=t`/`a=T`), placement requests (`a=p`), and the major delete selector families are now factored out of the top-level kitty parser flow, but payload decode/load, partial accumulation, image build, storage, placement graph updates, and delete/storage state still live in one broad module.
