...
- Kitty graphics has started its first real ownership split: the query path (`a=q`) is now factored out of the top-level kitty parser flow, but payload decode/load, partial accumulation, image build, storage, placement graph updates, and delete policy still live in one broad module.
