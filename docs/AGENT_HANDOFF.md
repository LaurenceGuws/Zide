...
- `TerminalSession` surface reduction is active: borrowed title/cwd/scrollback/selection seams are being cut back, terminal text export now lives in backend code instead of the widget, and FFI scrollback paging/counting is moving behind backend-owned export/metadata contracts so the bridge acts more like a marshaler than a second terminal implementation.
