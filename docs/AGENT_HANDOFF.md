...
- Current top-of-queue focus: continue the post-`src/app` giant file splits, starting with `src/editor/editor.zig`, then `syntax`, widget draw, renderer/font, and config parse, while collapsing low-value micro-files created by earlier extraction work.
