...
- The terminal FFI contract has now also survived an external peer-review host check in Flutty: the same widget/runtime layer worked across bridge-owned PTY and Flutter-owned PTY transport, and `main` now also exposes pending outbound input/report bytes for external transport so that PTY ownership does not have to imply a different host-input/report path.
