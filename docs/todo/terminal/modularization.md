...
- Active structural work should default to
  [vt_core_rearchitecture.md](vt_core_rearchitecture.md),
  not this file.
...
Layer check implemented via `zig build check-terminal-imports` (see `tests/checks/terminal_import_check.zig`).
...
- Follow-up (2026-03-26): extended `TerminalWorkspace.PollFrameMetrics` with explicit runtime-policy lifecycle/work-class fields and threaded that metadata through workspace polling metrics/logging, so terminal scheduling observability now uses the shared runtime vocabulary before budget behavior changes.
- Follow-up (2026-03-26): moved workspace poll policy from raw `has_input` booleans to explicit active/background `RuntimeIntent` values and started deriving hidden-warm background poll caps from shared runtime policy instead of only open-coded workspace math.
- Follow-up (2026-03-26): added cumulative workspace poll runtime counters and surfaced them through the existing terminal input-latency logging path, so active/background cooling can be observed over time instead of only via per-frame snapshots.
- Follow-up (2026-03-26): added resettable poll-counter epochs owned by `TerminalWorkspace` and reset them on real active-tab changes, so runtime measurements can compare post-focus windows instead of one lifetime aggregate.
- Follow-up (2026-03-28): extended `TerminalWorkspace` poll-counter epoch resets to tab create/close topology changes too, so lifecycle measurements stop blending one-background and many-background regimes into the same aggregate window.
