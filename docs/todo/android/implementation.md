...
- `APX-B18` architect review pending: Android refocus closure proof is complete
  on device with PM release `android-dev-2026.04.18.182005`; shipped in-prefix
...
### `APX15-M1` Tab-state ownership audit (`done`)
...
- **Navigation** (`AppShellNavigation`): owns tab **count** + **selected index** + `applySelectProductTerminalTab`; APX-B14 seed path stays on `forProductTerminalSlot(slot, index)` only.
- **Policy** (`AppShellTerminalViewPolicy`): owns the **coherent surface** — forward selection APIs + ordered `ProductTerminalTabDescriptor` list (stable id, label, index) validated against navigation count.
- **Chrome** (`ChromeController` / `ChromeBridge`): consumes descriptors for label text + click binding; maps index → sidebar button ids locally (layout seam); no duplicate label source outside policy-built list + `strings.xml` via `ProductTerminalTabDescriptors`.

### `APX15-M2` Policy descriptor model (`done`)

Queue line (exact):

- add app-shell tab descriptor model (stable id + label + index) and keep selected-index ownership on the same policy seam

### `APX15-M3` Chrome/sidebar consumption rewiring (`done`)

Queue line (exact):

- rewire chrome/sidebar tab binding to consume policy descriptors without behavior changes

### `APX15-M4` Restore + restart semantics lock (`done`)

Queue line (exact):

- verify restore seeding is side-effect free and distinct user selection still drives APX-B9 restart path only

### `APX15-M5` Docs + handoff sync (`done`)

Queue line (exact):

- sync authority docs and queue/handoff/entrypoint to APX-B15 tab-state model contract wording

### `APX15-M6` Validation + review packet (`done`)
...
### `APX17-M1` Runtime-link consumer audit (`done`)
...
- `Device proof: staged android-dev-2026.04.18.182005; shipped usr/bin/zide-pm list-available with ZIDE_PM_HOST_PLATFORM=android returned dev-baseline + zide-android-catalog-smoke; install zide-android-catalog-smoke exited 0; installed file /data/data/uk.laurencegouws.zide/files/usr/libexec/zide-pm/zide-android-catalog-smoke.sh is -rwxr-xr-x and prints zide-pm android-test-binary ok.`
- `Runtime support proof: readiness state launch_ready=true, artifact=zide-android-dev-prefix, version=sha256-498690c42bd7; app-owned .z bridge materialized at /data/data/uk.laurencegouws.zide/.z -> /data/data/uk.laurencegouws.zide/files/usr.`
...
**M1 outcome:** PM release `android-dev-2026.04.18.182005` contains expected candidate `zide-android-catalog-smoke` with install path `libexec/zide-pm/zide-android-catalog-smoke.sh`; Android can stage the release and materialize `.z`.
...
**M4 outcome:** Staged readiness state reports `launch_ready=true`, version `sha256-498690c42bd7`; `.z` and legacy support links materialize inside app-owned roots. Deploy + warm/cold start smokes passed with no `AndroidRuntime:E` output.
