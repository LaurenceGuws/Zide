...
- [ ] `LNX-01` Audit editor-baseline flows on Linux after the Windows sprint
  ...
  - 2026-03-29 note:
    - terminal shutdown hardening, child-exit shutdown capture, startup
      rollback, and close-on-child-exit decision-window proof are now tracked
      as cross-cutting runtime/lifecycle authority in:
      - `app_architecture/RUNTIME_ISOLATION_AND_RESOURCE_MANAGEMENT.md`
    - keep this Linux queue focused on Linux-specific native behavior and
      validation rather than generic lifecycle ownership
