...
- widget-side publication capture choreography is tighter too:
  ...
  and workspace frame pacing now consumes owner-shaped frame state rather than
  recomputing redraw/backlog/output-pressure from raw generation numbers
...
- config-owned screen/palette mutation is off the runtime surface too:
  ...
  contract
- interaction-owned input state and kitty-paste mechanics are off the runtime
  surface too:
...
- duplicate test-facing reply wrappers are dead too:
  `src/terminal/protocol/csi_reply.zig` no longer exposes `pty`-shaped wrapper
  entrypoints that only forwarded into its real writer-owned reply helpers,
