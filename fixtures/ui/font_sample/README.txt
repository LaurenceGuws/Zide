...
Deterministic fixture compare (sizes 12/14/16/20):
  ...
  tools/observability/rendering/font/font_sample_compare.sh

Strict header guard (dimensions/maxval):

  tools/observability/rendering/font/font_sample_compare.sh --strict-header

Refresh fixtures explicitly (opt-in):

  tools/observability/rendering/font/font_sample_compare.sh --update-fixtures
...
CSV output:
  ...
  tools/observability/rendering/font/font_sample_lcd_report.sh --csv

JSON output:

  tools/observability/rendering/font/font_sample_lcd_report.sh --json

PPM sanity validation:

  tools/observability/rendering/font/font_sample_validate_ppm.sh
...
- Captured sizes 12/14/16/20 with `tools/observability/rendering/font/font_sample_capture_lcd.sh`.
...
- If `tools/observability/rendering/font/font_sample_compare.sh --strict-header` fails after intentional
  rendering/capture pipeline changes, refresh fixtures with:
  `tools/observability/rendering/font/font_sample_compare.sh --update-fixtures --strict-header`
