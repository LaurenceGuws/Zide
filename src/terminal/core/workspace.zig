...
pub const TerminalWorkspace = struct {
    ...
    init_options: TerminalRuntimeShell.InitOptions,
    ...
    pub fn init(allocator: std.mem.Allocator, init_options: TerminalRuntimeShell.InitOptions) TerminalWorkspace {
        return .{
            .allocator = allocator,
    ...
    fn sessionAt(self: *TerminalWorkspace, index: usize) ?*TerminalRuntimeShell {
        if (index >= self.tabs.items.len) return null;
