...
const UploadResolution = struct {
    image_id: u32,
    control: common.KittyControl,
};

const QueryBuilder = struct {
    chunk: []u8,

    fn run(ctx: @This(), session: anytype, target_image_id: u32, ctl: common.KittyControl) common.KittyBuildError!void {
        const image = try KittyTransport.buildImage(session, target_image_id, ctl, ctx.chunk);
        session.allocator.free(image.data);
    }
};

pub const KittyProtocolOps = struct {
    pub fn parseAndDispatch(self: anytype, payload: []const u8) void {
        const log = app_logger.logger("terminal.kitty");
        var control = common.KittyControl{};
        var raw_kv = std.ArrayList(common.KittyKV).empty;
    ...
    fn handleDelete(self: anytype, control: common.KittyControl) void {
        if (!deleteKittyByAction(self, control)) {
            writeKittyResponse(self, control, resolveKittyImageId(control) orelse 0, false, "EINVAL");
        }
    }

    fn handlePlacementRequest(self: anytype, control: common.KittyControl) void {
        const image_id = resolveKittyImageId(control) orelse return;
        if (placement_mod.placeKittyImage(self, image_id, control)) |err_msg| {
    ...
    fn handleQuery(self: anytype, control: common.KittyControl, data: []const u8) void {
        if (handleQueryEarlyReply(self, control, data.len)) return;

        const image_id = resolveKittyImageId(control).?;
        var query_control = control;
        if (query_control.format == 0) query_control.format = 32;
        ...
        var chunk = KittyTransport.loadPayload(self, &query_control, data) orelse {
            writeKittyResponse(self, query_control, image_id, false, "EINVAL");
            return;
        };
    ...
    fn handleUpload(self: anytype, control: common.KittyControl, data: []const u8) void {
        const log = app_logger.logger("terminal.kitty");
        ...
        const final_data = KittyTransport.accumulate(self, upload.image_id, &upload_control, decoded) orelse {
            if (!upload_control.more) common.clearKittyLoading(common.kittyState(self), upload.image_id);
            return;
        };
    ...
    fn finishUpload(self: anytype, control: common.KittyControl, image_id: u32, final_data: []u8) void {
        const log = app_logger.logger("terminal.kitty");
        var upload_control = control;
        defer common.clearKittyLoading(common.kittyState(self), image_id);

        if (upload_control.format == 0) upload_control.format = 32;
        ...
        const image = KittyTransport.buildImage(self, image_id, upload_control, final_data) catch |err| {
            ...
            writeKittyResponse(self, upload_control, image_id, false, queryBuildErrorReplyMessage(err));
            return;
        };
        KittyStorageOps.store(self, image);
        if (upload_control.action == 'T') {
            if (placement_mod.placeKittyImage(self, image_id, upload_control)) |err_msg| {
    ...
    fn handleUploadSizeReply(self: anytype, control: common.KittyControl, image_id: u32, final_data: []u8) bool {
        if (KittyTransport.expectedDataBytes(control)) |expected| {
            if (final_data.len < expected) {
                var message = std.ArrayList(u8).empty;
                defer message.deinit(self.allocator);
                _ = message.writer(self.allocator).print("ENODATA:Insufficient image data: {d} < {d}", .{ final_data.len, expected }) catch {
    ...
    fn handleQueryEarlyReply(self: anytype, control: common.KittyControl, data_len: usize) bool {
    ...
    fn handleQueryPayloadPreflightReply(self: anytype, control: common.KittyControl, image_id: u32) bool {
    ...
    fn handleQueryPayloadSizeReply(self: anytype, control: common.KittyControl, image_id: u32, chunk_len: usize) bool {
        const log = app_logger.logger("terminal.kitty");
        if (KittyTransport.expectedDataBytes(control)) |expected| {
            if (chunk_len < expected) {
                var message = std.ArrayList(u8).empty;
                defer message.deinit(self.allocator);
                _ = message.writer(self.allocator).print("ENODATA:Insufficient image data: {d} < {d}", .{ chunk_len, expected }) catch |err| {
    ...
    fn queryBuildErrorReplyMessage(err: common.KittyBuildError) []const u8 {
        return switch (err) {
    ...
    fn inflateQueryChunk(self: anytype, control: common.KittyControl, image_id: u32, chunk: []u8) ?[]u8 {
    ...
    fn resolveUpload(self: anytype, control: common.KittyControl) ?UploadResolution {
        ...
        if (upload_control.more and kitty.loading_image_id == null) kitty.loading_image_id = final_image_id;
        if (upload_control.format == 0 and kitty.partials.getEntry(final_image_id) == null) upload_control.format = 32;
        return .{ .image_id = final_image_id, .control = upload_control };
    }
};

pub fn parseKittyControl(allocator: std.mem.Allocator, payload: []const u8, control: *common.KittyControl, raw_kv: *std.ArrayList(common.KittyKV)) []const u8 {
    var i: usize = 0;
    while (i < payload.len) {
        if (payload[i] == ';') {
            i += 1;
...
pub fn parseKittyValue(text: []const u8) ?u32 {
    const log = app_logger.logger("terminal.kitty");
    if (text.len == 0) return null;
    if (text.len == 1 and (text[0] < '0' or text[0] > '9')) return @intCast(text[0]);
...
pub fn parseKittySigned(text: []const u8) ?i32 {
    const log = app_logger.logger("terminal.kitty");
    if (text.len == 0) return null;
...
pub fn resolveKittyImageId(control: common.KittyControl) ?u32 {
    if (control.image_id) |id| return id;
    if (control.image_number) |id| return id;
    return null;
}

pub fn validateKittyControl(control: common.KittyControl) bool {
    switch (control.action) {
        't', 'T', 'p', 'd', 'q' => {},
        else => return false,
    }
    ...
    if (control.action == 'p' or control.action == 'T') {
        if (control.image_id == null and control.image_number == null) return false;
        if (control.virtual != 0 and (control.parent_id != null or control.child_id != null or control.parent_x != 0 or control.parent_y != 0)) return false;
...
fn deleteKittyPlacements(self: anytype, ctx: anytype, predicate: anytype) void {
    const kitty = common.kittyState(self);
    var idx: usize = 0;
    var changed = false;
    while (idx < kitty.placements.items.len) {
        if (predicate(ctx, self, kitty.placements.items[idx])) {
            placement_mod.KittyPlacementOps.markPlacementDirty(self, kitty.placements.items[idx], @src());
    ...
    if (changed) kitty.generation += 1;
}

fn deleteKittyByAction(self: anytype, control: common.KittyControl) bool {
...
fn deleteAllKittyPlacements(self: anytype) bool {
    const Ctx = struct {
        fn pred(_: @This(), _: anytype, _: common.KittyPlacement) bool {
            return true;
        }
    };
...
fn deleteKittyByIdSelector(self: anytype, id: ?u32, placement_id: u32, delete_images: bool) bool {
    ...
    const Ctx = struct {
        target_id: u32,
        target_pid: u32,
        fn pred(ctx: @This(), _: anytype, placement: common.KittyPlacement) bool {
...
fn deleteKittyByPoint(self: anytype, row: u16, col: u16, delete_images: bool) bool {
    const Ctx = struct {
        ...
        col: u16,
        delete_images: bool,
        fn pred(ctx: @This(), session: anytype, placement: common.KittyPlacement) bool {
...
fn kittyPlacementIntersects(placement: common.KittyPlacement, row: u16, col: u16) bool {
...
pub fn writeKittyResponse(self: anytype, control: common.KittyControl, image_id: u32, ok: bool, message: []const u8) void {
    const log = app_logger.logger("terminal.kitty");
