...
fn applySelectionActions(session: *terminal_runtime.TerminalRuntimeShell, actions: []const SelectionAction) void {
    for (actions) |action| {
        switch (action.op) {
            .start => terminal_selection.startSelection(session, action.row, action.col),
            .update => terminal_selection.updateSelection(session, action.row, action.col),
            .finish => terminal_selection.finishSelection(session),
            .select_range => terminal_selection.selectRange(
                session,
