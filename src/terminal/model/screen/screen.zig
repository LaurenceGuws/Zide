...
const write_ops = @import("write_ops.zig");
...
pub const Screen = struct {
    ...
    pub fn logDirtyRangeSemanticGaps(self: *const Screen, op: []const u8, start_row: usize, end_row: usize, start_col: usize, end_col: usize) void {
    ...
    pub fn markDirtyRangeWithOrigin(self: *Screen, op: []const u8, start_row: usize, end_row: usize, start_col: usize, end_col: usize) void {
        const rows = @as(usize, self.grid.rows);
        const cols = @as(usize, self.grid.cols);
        if (rows == 0 or cols == 0) return;
    ...
    pub fn writeCodepoint(self: *Screen, cp: u32, attrs: types.CellAttrs) void {
        write_ops.writeCodepoint(self, cp, attrs);
    }

    pub const WritePrep = write_ops.WritePrep;
    ...
    pub fn prepareWrite(self: *Screen) WritePrep {
        return write_ops.prepareWrite(self);
    }
    ...
    pub fn scrollRegionDownBy(self: *Screen, n: usize, blank_cell: types.Cell) void {
        edit_ops.scrollRegionDownBy(self, n, blank_cell);
    }

    pub fn scrollRegionDown(self: *Screen, count: usize, blank_cell: types.Cell) usize {
        return edit_ops.scrollRegionDown(self, count, blank_cell);
    }

    pub fn scrollUp(self: *Screen, blank_cell: types.Cell) void {
        edit_ops.scrollUp(self, blank_cell);
    }
};
...
pub fn mapDecSpecial(codepoint: u32) u32 {
    return write_ops.mapDecSpecial(codepoint);
}
pub fn setAutowrap(self: *Screen, enabled: bool) void {
    self.auto_wrap = enabled;
}
