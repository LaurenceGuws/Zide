...
pub const StatusBar = struct {
    ...
    pub fn draw(
        ...
        const text_y: f32 = y + (self.height - r.char_height) / 2;
        ...
        r.drawRect(0, @intFromFloat(y), @intFromFloat(mode_width), @intFromFloat(self.height), mode_bg_final);
        r.drawText(mode, text_x, text_y, if (mode_hover) Color.fg else Color.black);

        // File path
        var x: f32 = 88 * scale;
        if (file_path) |path| {
            const available = pos_start - 16 * scale - x;
            const result = common.drawTruncatedText(r, path, x, text_y, Color.fg, available);
        ...
        if (modified) {
            ...
            if (x + indicator_width <= pos_start - 8 * scale) {
                r.drawText(indicator, x, text_y, Color.orange);
            }
        }
        ...
        if (pos_hover) {
            ...
            r.drawRect(@intFromFloat(pos_start - 4 * scale), @intFromFloat(y + 2 * scale), @intFromFloat(pos_width + 8 * scale), @intFromFloat(self.height - 4 * scale), bg);
        }
        r.drawText(pos_str, pos_start, text_y, if (pos_hover) Color.fg else Color.comment);
    }
};
