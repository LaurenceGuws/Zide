...
pub fn draw(
    ...
    while (line_idx < total_lines and visual_row < visible_lines) : (line_idx += 1) {
        ...
        if (tokens.len == 0 and cache_helpers.shouldUseLargeFileFallback(widget.editor)) {
            const fallback_count = cache_helpers.buildLargeFileFallbackTokens(line_text, line_start, &fallback_tokens_buf);
...
pub fn drawCached(
    ...
    while (line_idx < total_lines and visual_row < visible_lines) : (line_idx += 1) {
        ...
        defer if (line_alloc) |owned| widget.editor.allocator.free(owned);
        const line_start = widget.editor.lineStart(line_idx);
        const line_text_hash = cache_helpers.hashLine(line_text);
