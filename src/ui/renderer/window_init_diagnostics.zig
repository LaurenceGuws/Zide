...
pub fn logGlContext(window: *sdl.SDL_Window) void {
    _ = window;
...
pub fn logWaylandNativeHandles(window: *sdl.SDL_Window) void {
...
pub fn logEglSurfaceContract(window: *sdl.SDL_Window) void {
    const driver = sdl_api.getCurrentVideoDriver() orelse return;
    if (!std.mem.eql(u8, driver, "wayland")) return;
