            ...
            .prepare_run_frame = struct {
                fn step(raw: *anyopaque) !?app_run_loop_driver.FrameSetup {
                    ...
                    return if (comptime forced_mode != null)
                        try app_prepare_run_frame_runtime.prepareFocused(rc.state, app_mode)
                    else
                        try app_prepare_run_frame_runtime.prepare(rc.state);
                }
            }.step,
            .update = struct {
