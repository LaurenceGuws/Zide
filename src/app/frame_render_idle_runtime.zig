...
var terminal_output_pressure_since: ?f64 = null;
var last_terminal_starve_log_time: f64 = 0.0;
...
pub fn handle(
    ...
    if (state.needs_redraw) {
        ...
        state.idle_frames = 0;
        terminal_output_pressure_since = null;
    ...
    if (terminal_output_pressure) {
        if (terminal_output_pressure_since == null) terminal_output_pressure_since = now;
        const pressure_ms = (now - terminal_output_pressure_since.?) * 1000.0;
        if (pressure_ms >= 100.0 and (now - last_terminal_starve_log_time) >= 0.1) {
            last_terminal_starve_log_time = now;
            const starve_log = app_logger.logger("terminal.ui.starve");
            starve_log.logf(
                .info,
                "pressure_ms={d:.2} idle_frames={d} needs_redraw={d} draw_seq={d} poll_seq={d}",
                .{
                    pressure_ms,
                    state.idle_frames,
