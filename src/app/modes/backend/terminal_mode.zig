...
pub const TerminalMode = struct {
    ...
    fn applyTabAction(self: *TerminalMode, allocator: std.mem.Allocator, action: shared.actions.TabAction) !bool {
        switch (action) {
            .create => {
                const id = self.next_tab_id;
                self.next_tab_id += 1;
                const title = try std.fmt.allocPrint(allocator, "Terminal {d}", .{id});
    ...
    fn findTabIndex(self: *const TerminalMode, id: shared.types.TabId) ?usize {
    ...
    fn activeTabIndex(self: *const TerminalMode) ?usize {
