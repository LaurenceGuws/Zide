...
fn drawMainSurfaces(state: anytype, shell: anytype, layout: layout_types.WidgetLayout, ctx: *anyopaque, hooks: Hooks) ?@import("../ui/widgets/common.zig").Tooltip {
...
fn handleCompletedPresent(state: anytype, shell: anytype, submission: anytype) void {
    ...
    app_logger.logger("renderer.present").logFields(.info, "frame_present", &.{
        ...
        .{ .key = "editor_texture_updates", .value = .{ .unsigned = trace.editor_texture_update_count } },
        .{ .key = "editor_texture_blits", .value = .{ .unsigned = trace.editor_texture_blit_count } },
...
pub fn draw(state: anytype, shell: anytype, ctx: *anyopaque, hooks: Hooks) void {
    ...
    _ = drawMainSurfaces(state, shell, layout, ctx, hooks);
