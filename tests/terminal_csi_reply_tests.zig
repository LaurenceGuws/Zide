...
test "CSI DA primary reply bytes" {
    ...
    defer pty.deinit();
    try std.testing.expect(csi_reply.writeDaPrimaryReply(&pty));
...
test "CSI DSR CPR reply bytes" {
    var pty = FakePty.init();
    defer pty.deinit();
    try std.testing.expect(csi_reply.writeDsrReply(&pty, 0, 6, 12, 34));
...
test "CSI DSR status reply bytes" {
    var pty = FakePty.init();
    defer pty.deinit();
    try std.testing.expect(csi_reply.writeDsrReply(&pty, 0, 5, 0, 0));
...
test "CSI DEC private DSR cursor reply bytes" {
    var pty = FakePty.init();
    defer pty.deinit();
    try std.testing.expect(csi_reply.writeDsrReply(&pty, '?', 6, 7, 9));
...
test "CSI DEC private DSR keyboard status reply bytes" {
    var pty = FakePty.init();
    defer pty.deinit();
    try std.testing.expect(csi_reply.writeDsrReply(&pty, '?', 26, 0, 0));
...
test "CSI DSR unsupported mode returns false and writes nothing" {
    var pty = FakePty.init();
    defer pty.deinit();
    try std.testing.expect(!csi_reply.writeDsrReply(&pty, 0, 999, 0, 0));
