...
test "CSI DA primary reply bytes" {
    ...
    defer pty.deinit();
    try std.testing.expect(csi_reply.writeDaPrimaryReplyWithWriter(&pty));
...
test "CSI DSR CPR reply bytes" {
    var pty = FakePty.init();
    defer pty.deinit();
    try std.testing.expect(csi_reply.writeDsrReplyWithWriter(&pty, 0, 6, 12, 34));
...
test "CSI DSR status reply bytes" {
    var pty = FakePty.init();
    defer pty.deinit();
    try std.testing.expect(csi_reply.writeDsrReplyWithWriter(&pty, 0, 5, 0, 0));
...
test "CSI DEC private DSR cursor reply bytes" {
    var pty = FakePty.init();
    defer pty.deinit();
    try std.testing.expect(csi_reply.writeDsrReplyWithWriter(&pty, '?', 6, 7, 9));
...
test "CSI DEC private DSR keyboard status reply bytes" {
    var pty = FakePty.init();
    defer pty.deinit();
    try std.testing.expect(csi_reply.writeDsrReplyWithWriter(&pty, '?', 26, 0, 0));
...
test "CSI DSR unsupported mode returns false and writes nothing" {
    var pty = FakePty.init();
    defer pty.deinit();
    try std.testing.expect(!csi_reply.writeDsrReplyWithWriter(&pty, 0, 999, 0, 0));
...
test "CSI DECRQM private reply bytes" {
    var pty = FakePty.init();
    defer pty.deinit();
    try std.testing.expect(csi_mode_query.writeDecrqmReplyWithWriter(&pty, true, 1004, .set));
...
test "CSI DECRQM ansi reply bytes" {
    var pty = FakePty.init();
    defer pty.deinit();
    try std.testing.expect(csi_mode_query.writeDecrqmReplyWithWriter(&pty, false, 20, .reset));
...
test "CSI DECRQM reply bytes cover representative Pm policy values" {
    ...
    for (cases) |case| {
        var pty = FakePty.init();
        defer pty.deinit();
        try std.testing.expect(csi_mode_query.writeDecrqmReplyWithWriter(&pty, case.private, case.mode, case.state));
