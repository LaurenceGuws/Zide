...
function getInnermostActiveFolderPath(activePath) {
    const lastSlash = activePath.lastIndexOf("/");
    if (lastSlash <= 0)
        return "";
    return activePath.slice(0, lastSlash);
}
function renderTreeNode(node, activePath, expandedPaths, innermostActiveFolderPath) {
    const dirEntries = Array.from(node.dirs.values()).sort((a, b) => a.name.localeCompare(b.name));
    const fileEntries = node.files
        .slice()
        .sort((a, b) => a.path.localeCompare(b.path));
    const activeDirIndex = dirEntries.findIndex((child) => activePath === child.path || activePath.startsWith(`${child.path}/`));
    const activeFileIndex = fileEntries.findIndex((child) => activePath === child.path);
    const activeChildIndex = activeDirIndex >= 0
        ? activeDirIndex
        : activeFileIndex >= 0
...
export function renderTreeMarkup(model, activePath, expandedPaths) {
