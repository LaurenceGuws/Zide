...
import type { AppShell, AppState, SearchHit } from "../shared/types.js";
...
export function installDocSearch(args: {
  ...
  function setSearchUi(next: {
    query: string;
    open: boolean;
    status: AppState["textSearch"]["status"];
    ...
    try {
      ...
      while (true) {
        ...
        while (newlineIndex >= 0) {
          ...
          if (line) {
            const payload = JSON.parse(line) as SearchHit | { error: string };
            if ("error" in payload) {
  ...
  shell.globalSearchEl.addEventListener("keydown", (event: KeyboardEvent) => {
    if (event.key === "Escape") {
      closeModal();
  ...
  shell.globalSearchResultsEl.addEventListener("click", (event: MouseEvent) => {
