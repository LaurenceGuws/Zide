...
if [ "$legacy_resolved_name" != "$resolved_name" ]; then
    python3 tools/editor_theme_import.py --remove-generated "$legacy_resolved_name" >/tmp/zide_theme_cleanup.log
...
python3 tools/nvim_resolved_theme_compare.py --summary "$artifact"
