...
- Modes: DECCOLM 80/132 column (gated by DEC mode 40), DECSCNM reverse video
...
- 80/132 column mode (DECCOLM, CSI ?3 h/l): currently treated as a no-op for column changes; we clear/home on set only. Full DECCOLM semantics are deferred.
...
- Public terminal compatibility contract and initial `zide` terminfo entry landed; broader terminfo parity sweep remains tracked under `TERM-01`.
