...
Purpose: define the high-level ownership and scope for the docs explorer tool
 and its relationship to Zide.
...
- the runtime is now a small static app with TypeScript source and generated
  browser ESM output owned by the standalone docs-explorer repo
...
- keep Zide's public docs/release entrypoints aligned with the standalone tool
  so repo docs do not drift from the actual explorer surface
