...
```mermaid
flowchart TD
    Change[Rendering change] --> Compare[tools/ui/font/font_sample_compare.sh]
...
- Default rule: if `tools/ui/font/font_sample_compare.sh` reports a mismatch, treat it
...
- Refresh workflow:
  - run `tools/ui/font/font_sample_compare.sh` and inspect mismatch outputs in
    `zig-cache/font_sample_compare/`;
  - if approved, copy updated captures into `fixtures/ui/font_sample/`;
  - rerun `tools/ui/font/font_sample_compare.sh` to confirm the repository is green.
...
- `tools/ui/font/font_sample_compare.sh --strict-header` remains green for default
...
- generated by `tools/ui/font/font_sample_lcd_snapshot.sh`
