...
Until the following are true, **Vulkan** and **Android rendering** (GLES/Vulkan
bootstrap) are still blocked by contract debt rather than pure implementation
effort. This does **not** block Android **native-host / platform** work that
does not assume a stable GPU backend—see `docs/todo/ui/renderer.md` and §
“Readiness (authoritative)” below.
...
If Vulkan or Android **rendering** bootstrap started today, the repo would still
be doing renderer surgery, not only a new backend module. The remaining blockers
are mainly:

- shared surface-phase semantics (flush discipline where surface work mixes
...
- renderer-hosted backend runtime shape (`Renderer.backend.runtime` now stores
  ...
  embeds it inline or exposes a backend-tagged storage surface, and runtime
  storage init/deinit now routes through backend runtime ops, but `Renderer`
  still owns the backend host surface itself)
- editor/sample ordering families still leaning on generic surface timing in
  places

### Gate status (code truth)

This table is the contract’s **as-implemented** read of the five adoption gates.
It must stay aligned with `src/ui/renderer/` (not aspiration).
...
| **3** | Backend-native runtime not a widening pattern on `Renderer` | **Not met** | `renderer_backend_host` still owns `kind` + `ops` + `runtime`. `backend_runtime_bundle.Bundle` now stores only the **selected** concrete backend runtime behind one opaque handle and no longer exposes a backend-tagged storage surface, and runtime storage init/deinit now routes through backend runtime ops. Shared code no longer reaches directly into `renderer.backend.ops` / `renderer.backend.kind`; it now terminates at `renderer_backend_host.zig`. But `Renderer` still owns that backend host surface itself. Adding Vulkan/Android rendering would still pressure this renderer-owned host story unless runtime becomes more backend-owned. |
...
- inside that row-band/content lane, the next remaining local split is now
  specifically the final styled-text emission edge:
  - fallback path still emits through direct text/decor functions in
    `editor_widget_draw_text.zig`
  - cached/list path emits the same semantics through draw-list text/rect
    ops there
- traversal, decoration geometry, and expanded styled-text run splitting are
  now shared there; further local cleanup is only valid if it removes that
  final emitter split or changes the real surface/timing contract.
...
- that is slightly narrower once more: expanded styled-text run splitting
