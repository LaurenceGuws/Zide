...
| `host/ui/WidgetHostAssemblyContext.java` | Good | Immutable dependency bundle for `ProductTerminalWidgetAssemblyHost` (progressive refs via suppliers where needed). | Extend only with scoped widget-host batches. |
| `host/ui/ProductTerminalWidgetAssemblyHost.java` | Good | `WidgetAssembly.Host` implementation; `ZideActivity` builds `WidgetHostAssemblyContext`. | Keep delegation-only; widget policy remains in existing harness controllers. |
...
| `host/ui/ProductHostDeclaredTerminalWidgetSlot.java` | Good | Explicit **host-declared slot** source for startup: `forCurrentProductHarness()` (catalog default today). `ZideActivity` reads it once, passes it into `AppShellTerminalHostSelectionContext.forProductHostStartup`, and threads the same value through interaction / widget host / composition wiring. | Vary when product host declares a non-default slot; keep one source before context construction. |
| `host/ui/AppShellTerminalHostSelectionContext.java` | Good | Startup **selection context**: one instance per `ZideActivity` onCreate path bundles `DeclaredTerminalWidgetSlotCatalog` + selected routing slot + `AppShellTerminalSelectionPolicy` after `forDeclaredHostSlot` on the host-declared slot from `ProductHostDeclaredTerminalWidgetSlot`; `WidgetAssembly.assemble(Host, context)` consumes it so assembly does not re-run selection resolution. | Extend when multi-slot startup varies host-declared slot; keep one context instance per startup path. |
