...
**APX-B17:** Readiness stamp carries `runtime_support_links` from the install-time manifest snapshot; `UserlandRuntimeSupportLinks` applies the same fragment during install and re-applies from the stamp before first native session `restart()` (app-owned package roots such as `/data/user/0/...` and `/data/data/...` are allowed).
