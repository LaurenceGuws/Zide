...
| `host/ui/AppShellTerminalSelectionPolicy.java` | Good | App-shell **selection** policy: which `TerminalWidgetSlotId` is selected for product terminal routing (today `PRIMARY` only). `WidgetAssembly` and `ZideActivity` resolve slot through this owner before `AppShellNavigation.forProductTerminalSlot`. | Extend when additional slots activate; pair with `checkActiveProductTerminalSlot` / mapping changes; no tab UI in this type. |
