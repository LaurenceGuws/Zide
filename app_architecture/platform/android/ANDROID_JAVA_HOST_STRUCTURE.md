...
| `host/ui/AppShellTerminalSelectionPolicy.java` | Good | App-shell **selection** policy: which `TerminalWidgetSlotId` is selected for routing; backed by `DeclaredTerminalWidgetSlotCatalog` + `checkActiveProductTerminalSlot`. `WidgetAssembly` and `ZideActivity` resolve slot through this owner before `AppShellNavigation.forProductTerminalSlot`. | Extend when additional slots activate; pair with catalog + mapping changes; no tab UI in this type. |
