...
3. One `TerminalWidgetCompositionAssembly.compose(ProductHostDeclaredTerminalWidgetSlot, interaction, surfaceJoin)` per
   instance → `TerminalWidgetInstance` (unwraps to `TerminalWidgetSlotId` for `checkActiveProductTerminalSlot`).
...
| `host/ui/ProductTerminalWidgetAssemblyHost.java` | Good | `WidgetAssembly.Host` implementation; unwraps `WidgetHostAssemblyContext.hostDeclaredTerminalWidgetSlot` for `terminalWidgetSlot()`. | Keep delegation-only; widget policy remains in existing harness controllers. |
...
| `host/interaction/InteractionCallbacks.java` | Good | Functional callback adapter into `host/interaction/InteractionAssembly`; stores `ProductHostDeclaredTerminalWidgetSlot` at startup, unwraps in `terminalWidgetSlot()`. | Keep adapter-only; avoid moving interaction behavior into this adapter. |
