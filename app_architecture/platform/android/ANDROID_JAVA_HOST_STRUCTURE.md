...
| `host/ui/ProductHostDeclaredTerminalWidgetSlot.java` | Good | Immutable **host-declared slot value** for startup: `forCurrentProductHarness()` wraps catalog default (`PRIMARY` today). `ZideActivity` holds one instance, passes it to `AppShellTerminalHostSelectionContext.forProductHostStartup`; `terminalWidgetSlot()` supplies the enum only at API boundaries. | Extend factory when host declaration varies; keep value type at startup seam. |
